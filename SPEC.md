...
### F4 克隆（拉取）到本地
- F4.1 克隆对话框字段：
  - 目标路径（文本输入；提供"使用默认根目录自动生成"按钮：`<默认根目录>/<owner>__<repo>`）；
  - 分支 / 标签选择（下拉，数据来自 `GET /repos/{owner}/{repo}/branches` 与 `/tags`，默认选中默认分支）；
  - 浅克隆选项（`--depth 1`，默认不勾选）。
- F4.2 同一仓库允许克隆到多个不同路径；路径重复（已存在且非空目录）时拒绝并提示。
...
- F5.6 补充：新建仓库（名称、描述、私有开关），创建成功后可立即克隆。
  ...
  "clones": [
    {
      ...
      "private": false,
      "paths": [
        { "path": "D:\\github-clones\\octocat__hello-world",
